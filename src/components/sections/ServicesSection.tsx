    ...
    <section id="services" className="py-20">
      <div className="container mx-auto px-6">
        <div className="mx-auto max-w-3xl text-center">
          <h2 className="mb-2 text-4xl font-bold text-sciscribe-navy">Our Services</h2>
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
          {services.map((service, index) => (
            <div
              key={index}
              className="animate-fade-in flex h-full flex-col rounded-lg border border-gray-100 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md"
              style={{ animationDelay: `${index * 0.15}s` }}
            >
              <div className="mb-4 inline-flex h-12 w-12 items-center justify-center rounded-lg bg-sciscribe-light text-sciscribe-navy">
                <service.icon size={24} />
              </div>
              <h3 className="mb-3 text-xl font-bold text-sciscribe-navy">{service.title}</h3>
              <p className="mb-6 flex-grow text-sciscribe-navy/80">{service.description}</p>
              <Button
                variant="ghost"
